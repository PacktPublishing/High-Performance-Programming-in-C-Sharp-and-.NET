namespace CH10_DataAccessBenchmarks.Data
{
	...
	public class DapperDotNet : IDisposable
	{
		...
		public IEnumerator<T> ExecuteReader<T>(string sql) where T : class
		{
			try
			{
				_dbConnection.Open();
		...
		public string GetProductNameSP(string productName)
		{
			try
			{
				...
				return _dbConnection.Query<Product>($"GetProductName", parameters, commandType: CommandType.StoredProcedure).First().ProductName;
			}
			finally
			{
				if (_dbConnection != null && _dbConnection.State == ConnectionState.Open)
