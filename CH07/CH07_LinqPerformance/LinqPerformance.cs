namespace CH07_LinqPerformance
{
	...
	public class LinqPerformance
	{
		...
		public static T Between<T>(Func<T> action)
		{
			return action();
		}

		static bool IsBetween(string param1, char param2, char param3)
		{
			var character = param1.ToCharArray()[0];
			return ((character >= param2) && (character <= param3));
		}

		static bool Between(string param1, char param2, char param3, Func<string, char, char, bool> IsBetween)
		{
			return IsBetween(param1, param2, param3);
		}

		[Benchmark]
		public void LinqWithParameters()
		{
			var data = (from p in _people.ToList()
						where Between(p.LastName, 'A', 'G', IsBetween)
						select p).ToList();
		}

		[Benchmark]
		public void LinqWithLocalVariable()
		{
			char first = 'A';
			char last = 'G';
			var data = (from p in _people.ToList()
						where Between(() =>
						{
							var character = p.LastName.ToCharArray()[0];
